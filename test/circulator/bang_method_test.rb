...
class BangMethodTest < Minitest::Test
  describe "bang method variants" do
    ...
    describe "FLOW_VARIANTS contract" do
      it "has a define method for every variant" do
        Circulator::FLOW_VARIANTS.each_key do |variant|
          method_name = ["define", variant, "flow_method"].compact.join("_")
          assert_respond_to BangMethodSampler, method_name, "Missing #{method_name} for variant #{variant.inspect}"
        end
      end
    end
